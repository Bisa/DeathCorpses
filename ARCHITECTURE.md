...
**AnyCPU/MSIL output:** All three assemblies must be compiled with `PlatformTarget=AnyCPU` so the PE header's `Machine` field stays `I386` (the MSIL marker), not `AMD64`. Nix's `buildDotnetModule` passes `--runtime linux-x64` by default, which would otherwise stamp the assembly as AMD64-only and break loading on ARM64 .NET runtimes (notably macOS native ARM builds of VS — see issue #5). The override is set both in the csproj files (`<PlatformTarget>AnyCPU</PlatformTarget>`) and on the `dotnet build` command line (`/p:PlatformTarget=AnyCPU` in `dotnetBuildFlags`). Verify with `file result/.../deathcorpses.dll` — it should report `PE32 ... Intel 80386`, not `PE32+ ... x86-64`.
