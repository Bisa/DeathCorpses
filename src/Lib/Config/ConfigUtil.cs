...
namespace DeathCorpses.Lib.Config
{
    internal static class ConfigUtil
    {
        public static object LoadConfig(ICoreAPI api, Type type, ref object config, out int loadedVersion, ILogger logger)
        {
            ...
            var jsonConfig = api.LoadModConfig<Dictionary<string, JToken>>(configAttr.Filename) ?? new Dictionary<string, JToken>();
            ...
            foreach (var prop in configProps)
            {
                ...
                {
                    prop.SetValue(config, ConvertType(ReadValue(element), prop.PropertyType, defaultValue));
                }
                else
                {
        ...
        // Config files are saved as { "Prop": { "Value": x, "Default": y, ... } }
        // When loaded as Dictionary<string, JToken>, each entry is a JObject with a "Value" field.
        private static object? ReadValue(JToken token)
        {
            if (token is JObject jobj && jobj.TryGetValue("Value", out var val))
                return val.ToObject<object>();
